...
int main(int argc, char* argv[]){
     ...
     if(argc != 10 && argc != 2){
				...
				cout << "Usage: ./exe {CLUSTER_FILE} {CONCORDANT_FILE} {UNIQUE SORTED FILE} {Lavg} {ReadLen} {Lambda} {Perr} {Limit} {Tolerance} {Verbose (Y or N)} {Max Chr Number} {MaxUniqueValue} {MinScaledUniqueness}\n"; // {Output Format}\n";
                cout << "   OR: Pass parameter file of the following format:" << endl;
	...
	int numIgnored = 0;
	double MAX_UNIQUE_VALUE;
	double MIN_SCALED_UNIQUE;
...
//	string Y1_INIT;
	...
	for(int chrNumber = 1; chrNumber<=MAX_CHR_NUMBER; chrNumber++){
		...
		while(	clusterFile.getline(Y1_INIT,15000000) ){
			...
			if(localChr == chrNumber && localType <=1 ){
				...
				*/
				...
				outFile1 << clusterName << "\t" << type  << "\t" << numDiscordants << "\t" << bestA << "\t" << bestB << "\t" << NUMCC_L << "\t" << NUMCC_R << "\t" << MAP_L << "\t" << MAP_R << "\t" << code << endl;
	...
	if(uniqErrors > 0){cout << "***NOTICE: There were " << uniqErrors << " uniqueness coverage errors. The missing values were replaced with the Max Unique Value parameter." << endl;}
