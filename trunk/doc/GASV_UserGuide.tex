...
\begin{itemize}
\item {\bf Default Value}: \verb+0.0001+
\end{itemize}
...
\begin{itemize}
\item {\bf Default Value}: \verb+0.3+
\end{itemize}
\end{description}
...
\begin{itemize}
\item {\bf Default Value}: \verb+100+ {\scriptsize NOTE: It is highly advisable to lower this value to reduce runtime, if possible.}
\end{itemize}
...
\begin{itemize}
\item {\bf Default Value}: \verb+false+
\end{itemize}
\item {\bf Translocations Only (TransOnly)}: setting this value to \verb+true+ will process translocations ONLY, ignoring all other types. {\bf WARNING: This operation can take hours.}
\begin{itemize}
\item {\bf Default Value}: \verb+false+
\end{itemize}
\item {\bf Likelihood Ratio Threshold (LRThreshold)}: Users can input a numerical value or the string \verb+all+. GASVPro will only return clusters with a LR greater than or equal to the input value (See section~\ref{LRexp} for explanation of LR). If \verb+all+ is given, all clusters will be printed.
\begin{itemize}
\item {\bf Default Value}: \verb+all+
\end{itemize}
