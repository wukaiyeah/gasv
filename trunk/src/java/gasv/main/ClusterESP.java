...
public class ClusterESP{
	...
	*/
	...
	public static int overlap(Clone Clone1,Clone Clone2){
		...
		PolyDefault p1 =(PolyDefault) Clone1.makeTrapezoid(); 
		PolyDefault p2 =(PolyDefault) Clone2.makeTrapezoid();
	...
	*/
	...
	/*public static boolean clique(PriorityQueue<Integer> Q, ArrayList<ArrayList<Integer>> Overlaps){
		boolean value = false;
		PriorityQueue<Integer> Temp = new PriorityQueue<Integer>();
		Temp.addAll(Q);
		int size = Temp.size();
		...
		for(int i = 0; i<size; i++){
			if(Overlaps.get(Temp.poll()).size() == size){
				marked++;
			}
		}
		if(marked == size){
	...
	private static void clusterHelper(ArrayList<BreakRegion> c, String directionString, 
		...
		*/
		...
		/*if (numClones > 1000) {
			Out.print2("clusterHelper() benchmarking for " + numClones
					+ " clones in direction " + directionString 
					+ ": after computing the clusters, time: " + System.currentTimeMillis());
		}
		*/
			...
			if (curCluster.getSize() <= GASVMain.MAX_CLUSTER_SIZE) {
				if (curCluster.getSize() <= GASVMain.MAX_CLIQUE_SIZE) {
					...
					if (curCluster.getSize() > 1000) {
						Out.print2("clusterHelper() benchmarking for cluster of size " 
								+ curCluster.getSize()
								+ ": before computing clique, time: " + System.currentTimeMillis());
					}
					*/
					...
					if (curCluster.getSize() > 1000) {
						Out.print2("clusterHelper() benchmarking for cluster of size " 
								+ curCluster.getSize()
								+ ": AFTER computing clique, time: " + System.currentTimeMillis());
					}
					*/
				} else {
	...
	private static boolean intersect(CGH cgh, Cluster cluster) {
		...
		if (cgh instanceof PairedCGH) {
			...
			// only if non-translocation, need to orient rectangle other way too
			// and also check this for intersection
			/*PolyDefault cghPoly2 = null;
			if (!pcgh.isTranslocation()) {
				cghPoly2 = new PolyDefault();
				cghPoly2.add(pcgh.getCoord3(), pcgh.getCoord1());
				cghPoly2.add(pcgh.getCoord4(), pcgh.getCoord1());
				cghPoly2.add(pcgh.getCoord4(), pcgh.getCoord2());
				cghPoly2.add(pcgh.getCoord3(), pcgh.getCoord2());
			}*/
	...
	/*private static ArrayList<String> findSplitReads(Cluster cluster) {
		ArrayList<String> splitReadStrings = null;
		ArrayList<Clone> splitReadClones = cluster.getSplitReads();
		if (splitReadClones != null && !splitReadClones.isEmpty()) {
			splitReadStrings = new ArrayList<String>();
			for (int i=0; i<splitReadClones.size(); ++i) {
				Clone curClone = splitReadClones.get(i);
				splitReadStrings.add(curClone.getName());
			}
		}

		return splitReadStrings;
	}*/
		...
		for(int i = 0; i<GASVMain.NUM_CHROM; i++){
			...
			for(int j = i; j<GASVMain.NUM_CHROM; j++){
				...
				do {
					...
					// print and clear at every window to save memory
					// TODO could put htis in clusterHelper instead to save even more memory??
					/*
					if (usePairedCGH) {
						printCGHClusterEntries(cghPairMap);
						clearCGHClusterMap(cghPairMap);
