...
public class Clone extends BreakRegion{
	...
	public Clone(String NAME, int CHRX, int CHRY, double X, double Y,int LMIN, int LMAX, int XLEN, int YLEN){
		name = NAME;
			...
			if (isTriangle()) {
				...
				*/
				...
				Out.print2("WARNING!!!  Encountered ESP: " 
						...
						+ Lmin);
				if (Lmin >= Lmax) {
					Out.print("ERROR: Lmin has been increased to >= Lmax for ESP: " + name 
							+ " so halting program!");
					System.exit(-1);
