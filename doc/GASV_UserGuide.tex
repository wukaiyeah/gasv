...
\\GASV (Version 2.0) and GASVPro (Version 1.2)}
%\author{The Author}
\date{August 15, 2012}                                           % Activate to display a given date or no date
...
\
\noindent Version: 1.2\\
\noindent Version Date: October 12, 2012
...
\end{itemize}
\item {\bf JAVAPREFIX:} The command to execute a jar file. (Note: The options \texttt{-Xms} and  \texttt{-Xmx} specify the memory usage of java and can be changed as necessary.)
\item Optional:
\begin{itemize}
...
\item {\bf OUTPUT:} Specifies the output format of cluster files (See Section~\ref{sec:GASVProHQOutput}.) (Default: Intervals) 
...
\texttt{GASVPro-HQ.sh} produces a number of, only a few of which are the ``final SV predictions". The following files are produced by running \texttt{GASVPro-HQ.sh}:
\begin{framed}
...
The GASVPro clusters files are in the GASV clusters format specified in the \texttt{OUTPUT} optional flag (default is intervals), but have an extra column for the log-likelihood ratio of a prediction. 
\subsection{GASVPro}
...
OUTPUT=NULL             #desired GASV cluster output (Default: intervals)
...
\item \textbf{CLEAN:} Clean/Remove working directory \texttt{MCMCTMPDIR} (Default: True). The MCMC code will generate many files during its execution. Although the final set of predictions will be reported, some users may with to retain this temperadirectory.
\end{itemize}
\end{itemize}
...
chmod 777 GASVPro-HQ.sh
\end{Verbatim}
Now it is possible to run \texttt{GASVPro.sh}. Initiate the pipeline by running 
\begin{Verbatim}[frame=single]
./GASVPro.sh
\end{Verbatim}
...
\noindent Let the probability that a given cluster $C$ {\em is} a variant be given by $P_v$ and the probability that $C$ {\em is not} a variant be given by $P_{nv}$.  The Log Likelihood Ratio therefore is $Log(P_v) -Log(P_{nv})$. The rest of the information in the \verb+.clusters+ file is identical to the input file. \label{LRexp}
